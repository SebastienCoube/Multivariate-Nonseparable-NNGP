...
x = rnorm(100*n_loc)
w = vecchia_blocks_solve(x, vecchia_blocks, n_time)


clust = kmeans(locs, 80)
w_split = lapply(split(w, rep(seq(n_loc), 100)), function(x)split(x, clust$cluster))
var_tag_split = split(var_tag, clust$cluster)
mapply(function(x, y)tapply(x, y, mean), w_split, var_tag_split)
