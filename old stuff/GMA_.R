...
#  i = 3;j =3;n=3;position_in_lower_tri(i, j, n)
  ...
  block_lower_tri_idx = lower_tri_idx(n, T)
  expand_covmat_into_blocks = function(covmat_coeffs, n, block_lower_tri_idx, sparse = F)
  {
    non_zero_idx  =which(covmat_coeffs[,1]!=0)
    idx = block_lower_tri_idx[non_zero_idx,]
    if(!sparse)blocks = lapply(seq(ncol(covmat_coeffs)), function(i)
    {
      M = matrix(0, n, n)
      M[idx]= covmat_coeffs[non_zero_idx, i]
      M = t(M)
      M[idx]= covmat_coeffs[non_zero_idx, i]
      M
    })
    if(sparse)blocks = lapply(seq(ncol(covmat_coeffs)), function(i)
    {
      M = Matrix::sparseMatrix(i = block_lower_tri_idx[,2], j = block_lower_tri_idx[,1], x = covmat_coeffs[non_zero_idx, i], symmetric = T)
    })
    blocks  
  }
  
  expand_block_toeplitz_covmat = function(covmat_coeffs, n, block_lower_tri_idx)
  {
    blocks = expand_covmat_into_blocks(covmat_coeffs, n, block_lower_tri_idx)
    block_idx_matrix = toeplitz(c(seq(ncol(covmat_coeffs))))
    res=  matrix(0, n*ncol(covmat_coeffs), n*ncol(covmat_coeffs))
    for(i in seq(ncol(block_idx_matrix)))
    {
      for(j in seq(ncol(block_idx_matrix))){
        res[seq(((i-1)*n+1), (i)*n), seq(((j-1)*n+1),(j)*n)]=blocks[[block_idx_matrix[i,j]]]
      }
    }
    res
  }
...
#####  
#####  multiply_vector_block_toeplitz = function(v, covmat_coeffs, n, idx_mat)
#####  {
#####    block_idx_matrix = toeplitz(c(seq(ncol(covmat_coeffs))))
#####    res=  rep(0, n*ncol(covmat_coeffs))
#####    for(i in seq(ncol(block_idx_matrix)))
#####    {
#####      for(j in seq(ncol(block_idx_matrix)))
#####      {
#####        res[(1+(i-1)*n):(i*n)] = res[(1+(i-1)*n):(i*n)] + Multiply_vector_compressed_symmat(
#####          v = v[(1+(j-1)*n):(j*n)], compressed_symmat = covmat_coeffs[,block_idx_matrix[j, i]], 
#####          idx_mat = idx_mat, n = n
#####          )
#####      }
#####    }
...
#####  }
#####  
#####  multiply_vector_block_toeplitz = function(v, covmat_coeffs, n, block_lower_tri_idx)
#####  {
...
#####  }
  ...
  multiply_vector_block_toeplitz = function(v, blocks, n)
  {
    k = length(blocks)
    v_ = matrix(v, ncol = k)
    res = matrix(0, n, k)
    for(i in seq(k))
    {
      ...
      toeplitz_idx[i]=1
      res = res + blocks [[i]] %*% v_ %*% toeplitz(toeplitz_idx)
    }
    res
  }
  ...
  v = rnorm(n*length(u))
  blocks = expand_covmat_into_blocks(covmat_coeffs, n, block_lower_tri_idx)
  multiply_vector_block_toeplitz(v = v, blocks, n)- 
  matrix(expand_block_toeplitz_covmat(covmat_coeffs, n, block_lower_tri_idx) %*% v, n)
...
##  {
##    tatat0= multiply_vector_block_toeplitz(v = v, blocks = expand_covmat_into_blocks(covmat_coeffs, n, block_lower_tri_idx), n=n)
##    return(0)
##  })
##  Sys.time()-t1
##  
...
##  {
##    tatat0=expand_covmat_into_blocks(covmat_coeffs, n, block_lower_tri_idx)
##    return(0)
##  })
##  Sys.time()-t1
##  
...
##                                                  nu_vec, 
##                                                  n_var
##                                                ), n = n, block_lower_tri_idx = block_lower_tri_idx))
##    return(0)
##  })
##  Sys.time()-t1
##  
...
##  Sys.time()-t1
  ...
  try(chol(expand_block_toeplitz_covmat(covmat_coeffs = covmat_coeffs, n = n, block_lower_tri_idx = block_lower_tri_idx)))
  #print(min(eigen(expand_covmat(covmat_compressed))$values))
  print(diag(expand_block_toeplitz_covmat(covmat_coeffs = covmat_coeffs, n = n, block_lower_tri_idx = block_lower_tri_idx)))
}
...
)
{
  ...
  for(i in seq(nrow(multiplier_and_variations$multiplier))){
    ...
    # covariance
    res$covmat[idx_] = res$covmat_without_rho[idx_] * rho_vec_[var_idx]
  }
}
